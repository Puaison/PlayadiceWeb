...
class CEvento
{
     ...
     * puÃ² effettivamente visualizzarla, sarÃ  possibile riprodurla, altrimenti verrÃ  mostrato un
     * messaggio d'errore. In caso la canzone sia visualizzata dall'artista stesso o da un moderatore,
     * sarÃ  possibile visualizzare funzionalitÃ  come la modifica o la rimozione.
     *
     * @param int $id l'identificativo della canzone da visualizzare.
     */
    static function show($id)
    {
        if(is_numeric($id)) // se nell'url Ã¨ effettivamente presente un id.
        {
            $vSong = new VSong(); // crea la view
            $user = CSession::getUserFromSession(); // ottiene l'utente dalla sessione
            $song = FPersistantManager::getInstance()->load(ESong::class, $id); // carica la canzone dell'id
            if($song) // se la canzone esiste, esegue il controllo di visibilitÃ 
            {
                $canSee = false; // variabile booleana che denota se l'utente puÃ² vedere la canzone o no
                $download = false; // variabile booleana che denota se l'utente puÃ² scaricare il brano

                if($user->getId() == $song->getArtist()->getId() || is_a($user, EModerator::class)) // se l'utente Ã¨ l'autore della canzone
                {
                    $canSee = true;
                    $download = true;
                }
                else if($song->isForAll()) // se Ã¨ per tutti...
                    $canSee = true;
                else if (get_class($user)!=EGuest::class) // se l'utente e' registrato...
                { // si effettuano gli altri controlli
                    if($song->isForRegisteredOnly() && get_class($user)!=EGuest::class) // se Ã¨ per i registrati e l'utente non Ã¨ guest...
                        $canSee = true;
                        ...
                        // se Ã¨ per i supporter e l'utente supporta l'artista della canzone...
                    {
                        $download = true;
                        $canSee = true;
                    }
     ...
     * se l'utente che vuole rimuovere il brano non Ã¨ l'autore del brano stesso.
     * @param int $id l'identificativo della cazone.
     */
